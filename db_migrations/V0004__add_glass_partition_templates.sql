...
CREATE TABLE IF NOT EXISTS t_p56372141_online_booking_integ.package_components (
    ...
    component_id INTEGER REFERENCES t_p56372141_online_booking_integ.glass_components(component_id),
...
('Лента', 'tape', '4918F монт. двуст.', 'основа вспен. акр. адгезив VHB, прозр., 6мм х 16,5м х 2мм', 'погм', 100.00),
('Заглушка', 'plug', 'FS G-z10', '40*30мм, СЕРАЯ', 'шт', 100.00),
('Петля BO5214350', 'hinge', 'BO5214350', 'для маятниковой стеклянной двери 750-1000мм до 100 кг', 'шт', 13200.00),
('Петля верхняя', 'hinge', 'AX-12DD', 'Верхняя SSS', 'шт', 1100.00),
...
INSERT INTO t_p56372141_online_booking_integ.glass_packages (
    ...
    'Перегородка ЦЕЛЬНОСТЕКЛЯННАЯ (Дверь Маятниковая)',
    'partition',
...
-- Связываем комплект с компонентами
WITH last_package AS (
    SELECT package_id FROM t_p56372141_online_booking_integ.glass_packages 
    WHERE package_name = 'Перегородка ЦЕЛЬНОСТЕКЛЯННАЯ (Дверь Маятниковая)'
...
SELECT 
    ...
    CASE 
        ...
        WHEN component_name = 'Лента' THEN 12.24
        WHEN component_name = 'Заглушка' THEN 8.00
        WHEN component_name = 'Петля BO5214350' THEN 2.00
        WHEN component_name = 'Петля верхняя' THEN 2.00
...
WHERE component_name IN (
    'Профиль к-т', 'Лента', 'Заглушка', 'Петля BO5214350', 'Петля верхняя',
    'Ось верхняя', 'Замок', 'Ручка-рейлинг', 'Стекло 10мм прозрачное', 
