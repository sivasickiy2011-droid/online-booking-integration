...
export function useExcelImport(packages: GlassPackage[], fetchPackages: () => void) {
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (typeof positionNum === 'number' && article && name) {
          ...
          console.log(`Main component #${positionNum}: ${article} - ${name}`);
          ...
          if (lastMainComponent) {
            ...
            console.log(`Alternative for ${lastMainComponent.article}: ${article} - ${name}`);
          }
        }
      }
  ...
  const downloadExcelTemplate = () => {
    ...
    const templateData = [
      ['ДКП-ПРИМЕР', '', 'Комплект для душевой кабины (пример)', '', '', '', '', '', '', '', ''],
      ['', '', '', '', '', '', '', '', '', '', ''],
      ['№', 'Группа 1', 'Артикул', 'Группа 2', 'Характеристика', 'кол-во', 'ед. изм.', 'цена', 'всего', 'тип', 'PIC'],
      [1, 'Крепление', 'GM-657', 'Крепление штанга-стекло', '[15*15мм, 90˚, PSS]', 1, 'шт', 1098.50, '', '', ''],
      ['', 'Крепление', 'GM-658', 'Крепление штанга-стекло', '[15*15мм, 180˚, PSS]', 1, 'шт', 769.23, '', '', ''],
      ['', 'Крепление', 'TG-30-14B', 'Крепление штанга-стекло', '[30*10мм PSS]', 1, 'шт', 850, '', '', ''],
      [2, 'Уплотнитель', 'PS-2', 'Уплотнитель универсальный', '[8мм, 135˚, б/цв, 2200мм]', 1, 'шт', 250, '', '', ''],
      ['', 'Уплотнитель', 'PS-11', 'Уплотнитель универсальный', '[8мм, универсальный, б/цв, 2200мм]', 1, 'шт', 184.62, '', '', ''],
      [3, 'Ручка', 'H-201', 'Ручка-скоба', '[L300мм, Хром]', 1, 'шт', 1846, '', '', ''],
      ['', '', '', '', '', '', '', '', '', '', ''],
      ['', '', '', '', '', '', '', '', '', '', ''],
      ['Инструкция:', '', '', '', '', '', '', '', '', '', ''],
      ['1. Строка 1: Артикул комплекта (ДКП-xxx) в столбце А', '', '', '', '', '', '', '', '', '', ''],
      ['2. Строка 3: Заголовки таблицы (не трогать)', '', '', '', '', '', '', '', '', '', ''],
      ['3. Основные компоненты: начинаются с номера в столбце № (1, 2, 3...)', '', '', '', '', '', '', '', '', '', ''],
      ['4. Аналоги: БЕЗ номера в столбце №, сразу после основного компонента', '', '', '', '', '', '', '', '', '', ''],
      ['5. Группа 1 - общая категория, Группа 2 - конкретное название', '', '', '', '', '', '', '', '', '', '']
    ];
    ...
    XLSX.writeFile(wb, 'template_komplekt.xlsx');
    
    toast({
      title: 'Шаблон скачан',
      description: 'Файл template_komplekt.xlsx готов к заполнению'
    });
  };
